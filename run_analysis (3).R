...
# function below will read txt files and covnert that into data.frame
...
#Run above function gettables with argument "feature.txt"
features <- gettables("features.txt")

#Function below will read data and build database
...
#run above function getdata with arguments"test"& feature
test <- getdata("test", features)
#repeat getdata with arguments"train"& feature
train <- getdata("train", features)

#Below function will save the resulting data in the results folder
saveresults <- function (data,name){
	print(paste("saving results", name))
	file <- paste(resultsfoldername, "/", name,".csv" ,sep="")
	write.csv(data,file)
}

### required activities for this course project###

## install Rpackage "plyr" ignore if already installed
...
#2) Extracts only the measurements on the mean and standard deviation for each measurement. 
mean_and_sd <- data[,c(1,2,grep("std", colnames(data)), grep("mean", colnames(data)))]
